...
export default function NewExpensePage() {
  ...
  // const [date, setDate] = useState(new Date().toISOString().split('T')[0]); // Removed date state
  ...
  useEffect(() => {
    Promise.all([
      ...
      setGroups(g || []);
      setUsers(u || []);
      if (session?.user?.id) setPaidBy(session.user.id);
  ...
  return (
    <div className="p-4 md:p-8 max-w-[800px]">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-t1">Add Expense</h1>
        <p className="text-sm text-t2 mt-0.5">Record a new expense to split with your team</p>
      </div>
